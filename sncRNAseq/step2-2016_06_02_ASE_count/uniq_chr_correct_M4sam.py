...
##  prepare sam for ase calculation (using ase_M_fast.py)
##  If cigar is splice junction, insertion, deletion (N, I, D), it generates new reads for them that have a new cigar, readLen+"M"
##  --> for missing seq, add "x", for missing qual, add "I"
##  usage : uniq_chr_correct_M4sam.py accepted_hits.sam > accepted_hits_uniq_chr_M.sam
...
def print_line(line):
    ...
    for j in sig:
        ...
        if j == "M":
            ...
            seq = first_seq[seq_start :seq_start +int(num)]+"x"*(len(first_seq)-int(num))
            col[9] = seq

            # update qual
            #qual = first_qual[seq_start :seq_start +int(num)]+"I"*(50-int(num))
            qual = first_qual[seq_start :seq_start +int(num)]+"I"*(len(first_seq)-int(num))
            col[10] = qual
